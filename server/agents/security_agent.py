...
def security_agent(state: AgentState) -> dict:
    ...
    # build user message — code + tool findings
    user_message = _build_user_message(code, filename, security_findings)

    try:
        issues = _call_llm(user_message, filename)
    except Exception as e:
        logger.error(f"Security agent LLM call failed: {e}")
        issues = []
...
def _build_user_message(code: str, filename: str, findings: list) -> str:
    ...
    # no backtick fences around code — avoids breaking if user code contains backticks
    return f"""
FILENAME: {filename}
...
def _call_llm(user_message: str, filename: str) -> list[Issue]:
    ...
    response = client.messages.create(
        ...
        max_tokens=4096,
        system=[{"type": "text", "text": SECURITY_PROMPT, "cache_control": {"type": "ephemeral"}}],
        messages=[
...
# extract first JSON array if there's extra text around it
    ...
    try:
        ...
        logger.warning(f"Security agent: JSON parse failed: {e} — attempting repair")
        try:
            parsed = json.loads(repair_json(raw))
            logger.info(f"Security agent: JSON repaired successfully")
        except Exception:
            logger.warning("Security agent: repair failed too")
            return []  
    ...
    for item in parsed:
        try:
            issue = Issue(
                ...
                agent_agreement=item.get("agent_agreement", ["security"]),
                cross_domain_notes=item.get("cross_domain_notes", None),
            )
            issues.append(issue)
        except Exception as e:
            # one bad issue shouldn't lose the whole response
            logger.warning(f"Skipping malformed issue from security agent: {e}")
            continue
